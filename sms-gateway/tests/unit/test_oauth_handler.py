...
def test_authorize_redirects_to_auth_uri(client, services):
    ...
    session_manager.get_pending_credential = AsyncMock(
        return_value={"auth_uri": "https://accounts.google.com/o/oauth2/auth?foo=bar"}
    )

    resp = client.get("/oauth/authorize?nonce=abc123", follow_redirects=False)

    assert resp.status_code == 307
    assert resp.headers["location"] == "https://accounts.google.com/o/oauth2/auth?foo=bar"


def test_authorize_returns_404_for_unknown_nonce(client, services):
    session_manager, _, _ = services
    session_manager.get_pending_credential = AsyncMock(return_value=None)

    resp = client.get("/oauth/authorize?nonce=unknown", follow_redirects=False)
...
# ── /oauth/connector/callback — nonce path ────────────────────────────────────

def test_callback_nonce_path_resumes_agent_and_sends_discord(client, services):
    ...
    session_manager.clear_pending_credential.assert_called_once_with("mynonce")


def test_callback_nonce_path_unknown_nonce_returns_400(client, services):
    session_manager, _, _ = services
    session_manager.get_pending_credential = AsyncMock(return_value=None)

    resp = client.get("/oauth/connector/callback?state=badnonce")
...
def test_callback_validation_state_calls_finalize_and_resumes(client, services):
    session_manager, agent_client, discord_sender = services
    session_manager.get_latest_pending_credential = AsyncMock(return_value=(
        "nonce-xyz",
        {
            "user_id": "user-2",
            "session_id": "sess-2",
...
def test_callback_validation_state_finalize_failure_returns_500(client, services):
    ...
    session_manager.get_latest_pending_credential = AsyncMock(return_value=(
        "nonce-xyz",
        {"user_id": "user-2", "session_id": "sess-2",
         "credential_function_call_id": "fc-2", "auth_config_dict": {}},
    ))
    ...
    with patch(
        ...
        resp = client.get(
            "/oauth/connector/callback?user_id_validation_state=opaque&connector_name=projects/p/locations/l/connectors/c"
        )
