...
def _personal_prompt() -> str:
    ...
    return (
        ...
        "For comprehensive inbox triage ('what are my action items', 'summarize my inbox'), use a single "
        "read_emails(max_results=20) call, then fetch_email() on the most relevant messages. "
        "Do NOT make multiple narrow read_emails calls with different query terms — one broad call is far more efficient.\n\n"
        ...
        "1. Call atomic_read_received_emails() FIRST — do not call any other tools before it. "
        "If it returns 'No new emails.', respond with <SUPPRESS RESPONSE> and stop. "
        "Otherwise call fetch_email(message_id) for each returned message ID.\n"
        ...
        "   - Invoice / deadline / delivery / action required → save to Drive and notify; "
        "     do NOT call create_async_task (email notification sessions lack Discord channel context — it will always fail)\n"
