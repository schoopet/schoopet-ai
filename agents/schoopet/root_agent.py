...
def _personal_prompt() -> str:
    ...
    return (
        ...
        "The agent runs iterative search loops (search → critique, up to 3 rounds), deduplicates against "
        "the existing collection, writes new candidates with status 'New — Pending Review', and returns a summary.\n\n"
        "**Scheduling workflow**: Before creating an async task for this agent, always confirm a research plan "
        "with the user first — category, location, output destination (Sheet ID or Doc ID), and any filters. "
        "Get explicit approval, then schedule. The deep research agent executes autonomously and does not "
        "ask for approval again.\n"
        "For recurring research (e.g. 'every week find new restaurants'), create a recurring async task "
        "that delegates here with the approved plan as context.\n\n"
